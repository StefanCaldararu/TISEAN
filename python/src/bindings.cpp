...
public:
  ...
  unsigned int ipoles() const { return model_->ipoles; }
  unsigned int mapoles() const { return model_->mapoles; }
  bool arimaset() const { return model_->arimaset != 0; }
  unsigned int order() const { return model_->order; }
  unsigned int size() const { return model_->size; }
  unsigned int realiter() const { return model_->realiter; }

  py::array_t<double> average() const {
    py::array_t<double> out((py::ssize_t)model_->dim);
    auto buf = out.mutable_unchecked<1>();
    for (unsigned int i = 0; i < model_->dim; i++)
      buf(i) = model_->average[i];
    return out;
  }
  ...
  py::array_t<unsigned int> coeff_index() const {
    unsigned int size = model_->size;
    py::array_t<unsigned int> out({(py::ssize_t)2, (py::ssize_t)size});
    auto buf = out.mutable_unchecked<2>();
    for (unsigned int i = 0; i < size; i++) {
      buf(0, i) = model_->aindex[0][i];
      buf(1, i) = model_->aindex[1][i];
    }
    return out;
  }
  ...
  py::array_t<double> xdiff() const {
    unsigned int dim = model_->dim, realiter = model_->realiter;
  ...
  py::array_t<double> iterate(unsigned long ilength, unsigned long seed) const {
    double **out = arima_model_iterate(model_, ilength, seed);
    py::array_t<double> result({(py::ssize_t)ilength, (py::ssize_t)model_->dim});
    auto buf = result.mutable_unchecked<2>();
    for (unsigned long n = 0; n < ilength; n++)
      for (unsigned int d = 0; d < model_->dim; d++)
...
arima_fit(py::array_t<double, py::array::c_style | py::array::forcecast> series,
	  unsigned int poles, unsigned int arpoles, unsigned int ipoles,
	  unsigned int mapoles, unsigned int iterations, double convergence)
{
  if (series.ndim() != 2)
    throw std::invalid_argument("series must be a 2D array of shape (dim, length)");
  if (iterations < 1)
  ...
  ARIMAModelError error = ARIMA_MODEL_OK;
  ARIMAModel *model = arima_model_fit(rows.data(), length, dim, poles, arpoles,
				       ipoles, mapoles, iterations, convergence,
				       &error);
  if (model == nullptr) {
    if (error == ARIMA_MODEL_ERR_ZERO_VARIANCE)
      throw std::invalid_argument("series has zero variance after differencing");
    throw std::invalid_argument(
	"poles must be >= 1 and < series.shape[1] after differencing (and, if "
	"the ARMA refinement is requested via arpoles/ipoles/mapoles, "
	"arpoles/mapoles must be too)");
  }
...
PYBIND11_MODULE(_tisean, m)
{
  ...
  py::class_<ARIMAModelWrapper>(arima_model, "ARIMAModel")
      ...
      .def_property_readonly("ipoles", &ARIMAModelWrapper::ipoles)
      .def_property_readonly("mapoles", &ARIMAModelWrapper::mapoles)
      .def_property_readonly("arimaset", &ARIMAModelWrapper::arimaset,
			      "Whether the ARMA refinement ran "
			      "(arpoles+ipoles+mapoles > 0)")
      .def_property_readonly("order", &ARIMAModelWrapper::order,
			      "poles actually used to index coeff/residuals: "
			      "poles if not arimaset, else max(arpoles, mapoles)")
      .def_property_readonly("size", &ARIMAModelWrapper::size,
			      "Number of coefficients per dim row")
      .def_property_readonly("realiter", &ARIMAModelWrapper::realiter,
			      "Number of ARMA refinement iterations actually run")
      .def_property_readonly("average", &ARIMAModelWrapper::average,
			      "Per-component mean subtracted while centering, "
			      "shape (dim,)")
      .def_property_readonly("series", &ARIMAModelWrapper::series,
			      "The differenced, centered series that was fit, "
			      "shape (dim, length)")
      .def_property_readonly("coeff", &ARIMAModelWrapper::coeff,
			      "AR(/MA) coefficients, shape (dim, size)")
      .def_property_readonly("coeff_index", &ARIMAModelWrapper::coeff_index,
			      "(component, lag) each coeff column belongs to, "
			      "shape (2, size); component < dim means the "
			      "series itself, dim <= component < 2*dim "
			      "(arimaset only) means the residual/MA term")
      .def_property_readonly("rms_error", &ARIMAModelWrapper::rms_error,
			      "RMS one-step forecast error per component, "
			      "shape (dim,)")
      .def_property_readonly("residuals", &ARIMAModelWrapper::residuals,
			      "One-step-ahead residual series, shape (dim, length)")
      .def_property_readonly("xdiff", &ARIMAModelWrapper::xdiff,
			      "Per-iteration RMS change of the residuals, "
			      "shape (realiter, dim)")
      .def_property_readonly("diffcoeff", &ARIMAModelWrapper::diffcoeff,
			      "Per-iteration RMS change of the coefficients, "
			      "shape (realiter,)")
  ...
  arima_model.def(
      "fit", &arima_fit, py::arg("series"), py::arg("poles") = 10,
      py::arg("arpoles") = 0, py::arg("ipoles") = 0, py::arg("mapoles") = 0,
      py::arg("iterations") = 50, py::arg("convergence") = 1.0e-3,
      "Difference (ipoles times) and fit a multivariate AR model to "
      "`series` (shape (dim, length)), of order `poles`.\n\n"
      "If arpoles+ipoles+mapoles > 0, refine it into a combined AR/I/MA fit "
      "of order (arpoles, ipoles, mapoles), iterating up to `iterations` "
      "times or until the residual RMS change drops below `convergence`.\n\n"
      "series is centered (and, if ipoles > 0, differenced) internally, the "
      "same way the arima-model CLI does it - the input array itself is not "
      "modified.");
