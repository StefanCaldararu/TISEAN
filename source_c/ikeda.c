...
#define WID_STR "iterate Ikeda map"
...
double a = 0.4, b = 6.0, c = 0.9, xinit = .68587, yinit = .65876;
...
void show_options(char *progname)
{
  ...
  fprintf(stderr, "\t-A parameter a [default 0.4]\n");
  fprintf(stderr, "\t-B parameter b [default 6.0]\n");
  fprintf(stderr, "\t-C parameter c [default 0.9]\n");
  ...
  fprintf(stderr, "\t-o output file name, just -o means ikeda.dat"
...
void scan_options(int argc, char **argv)
{
  ...
  if ((out = check_option(argv, argc, 'B', 'f')) != NULL)
    sscanf(out, "%lf", &b);
  if ((out = check_option(argv, argc, 'C', 'f')) != NULL)
    sscanf(out, "%lf", &c);
...
int main(int argc, char **argv)
{
  ...
  if (!length_set) {
    fprintf(stderr, "ikeda: -l is required\n");
    exit(1);
  }
  ...
  /* Iterate the Ikeda map. length!=0 (an explicit -l) is handled by the
     reentrant ikeda_generate() API (source_c/api/ikeda_api.c), shared
  ...
  if (length != 0) {
    series = ikeda_generate(a, b, c, xinit, yinit, length, ntrans);
    for (n = 0; n < (long)length; n++)
      fprintf(outf, "%.9e %.9e\n", series[2 * n], series[2 * n + 1]);
    ikeda_free(series);
  }
  else {
    double xo = xinit, yo = yinit, xn, yn, s, cs, ss;
    long nn;
